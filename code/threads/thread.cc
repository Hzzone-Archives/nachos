...
Thread::Thread(char* threadName){
    if(++thread_num > 128){
	cout << "current thread is more than 128" << endl;
        ASSERT(thread_num <= 128);
    }
    cout << "Create Thread: " << thread_num << endl;
    name = threadName;
    stackTop = NULL;
    stack = NULL;
    status = JUST_CREATED;
 ...
 void SimpleThread(int which)
{
    ...
    for (num = 0; num < 3; num++) {
	      cout << "thread " << kernel -> currentThread -> getName() << " priority: "
          << kernel -> currentThread -> getPriority() << " loops: " << num+1 <<" times\n";
...
/*    Thread *t = new Thread("forked thread");
    ...
    Thread* t[129];
    for(int i=0;i<129;i++){
	t[i] = new Thread("create threads");
    }

    //Thread *t1 = new Thread("1",2);
    //Thread *t2 = new Thread("2",1);
    //Thread *t3 = new Thread("3",4);
